...
/**
 * Gauge — a meter bent into an arc. Ticks up to the value are filled in the
 * current band's colour; the rest sit on a neutral track. The figure in the
 * middle is the one hero number of the mockup, so it's sans, not serif.
 */
export function Gauge({ value, dark = true, label = 'Body-stress index' }) {
  const sev = bandOf(value)
  const r = RISK[sev]
  const N = 40
  const R = 78
  const cx = 90
  const cy = 88
  const len = 14
  const track = dark ? 'rgba(150, 163, 189, 0.26)' : 'rgba(31, 36, 48, 0.12)'

  const ticks = []
  for (let i = 0; i < N; i += 1) {
    const t = i / (N - 1)
    const a = Math.PI * (1 - t) // sweep left → right
  ...
  return (
    <div className="relative mx-auto w-full max-w-[220px]">
      <svg
        ...
        aria-valuemin={0}
        aria-valuemax={100}
        aria-label={`${label} ${value} of 100, ${sev}`}
      >
        {ticks}
      </svg>
      <div className="absolute inset-x-0 bottom-0 text-center">
        <div className={`text-4xl font-extrabold leading-none ${dark ? 'text-frost' : 'text-ink'}`}>
          {value}
        </div>
        <div
          className={`mt-1 flex items-center justify-center gap-1.5 text-[11px] font-semibold ${
            dark ? 'text-mist' : 'text-slate-soft'
          }`}
        >
          <span className="h-1.5 w-1.5 rounded-full" style={{ background: r.hex }} />
